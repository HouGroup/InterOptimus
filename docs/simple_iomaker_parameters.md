...
- **`vasp_pair_selection`**（`IOMaker` 字段，决定提交多少套界面弛豫）：
  - **`each_match_lowest`**（**默认**）：每个 lattice **match** 只跑 **最低能量 termination** 对应的一套界面弛豫。
  - **`each_plane_lowest`**：每个 film/substrate **Miller 面组合** 保留一个最优对（与极图 / `area_strain` 分 bin 一致）；**旧名** `each_plane`、`per_plane` 仍可用。
  - **`all_optimized`**：`global_optimized_data` 中 **每一行 (match, term)** 都跑 VASP（若需尽量全跑，常配合调大 **`global_minimization_settings.term_screen_tol`**）。
  - 短别名：`each_match` 等同 `each_match_lowest`。
