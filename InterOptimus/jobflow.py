...
def _miller_hkl_lists_for_report(idx_data: Optional[dict]) -> Tuple[Optional[List[int]], Optional[List[int]]]:
    """
    Conventional Miller indices for pairs_summary / io_remote_summary.
    Falls back to primitive indices when conventional hkl is empty (shows as [] in JSON).
    """
    if not idx_data or not isinstance(idx_data, dict):
        return None, None

    def _norm(key: str) -> Optional[List[int]]:
        a = idx_data.get(key)
        if a is None:
            return None
    ...
    fc = _norm("film_conventional_miller") or _norm("film_primitive_miller")
    sc = _norm("substrate_conventional_miller") or _norm("substrate_primitive_miller")
    return fc, sc
...
def _serialize_unique_matches_millers_for_payload(idt: Any) -> List[Dict[str, Any]]:
    ...
    Used when rebuilding ``pairs_summary`` after VASP: ``area_strain`` rows repeat
    ``(match_id, term_id)`` for stereographic bins, so many terminations have **no**
    dedicated ``area_strain`` line and would otherwise show ``[]``.
    """
    if not idt or not isinstance(idt, (list, tuple)):
        return []
...
class GDVaspMaker(Maker):
    ...
    #: If set (see :meth:`InterOptimus.itworker.InterfaceWorker.patch_jobflow_jobs`), append
    #: ``MPStaticSet`` job(s) after the final GD relax so interface energies match the static protocol.
    gd_post_static_config: Optional[Dict[str, Any]] = None
    ...
    def save_final_data(self, saved_data):
        if self.gd_post_static_config:
            from InterOptimus.itworker import gd_final_post_relax_static_jobs

            cfg = dict(self.gd_post_static_config)
            return gd_final_post_relax_static_jobs(
                saved_data=saved_data,
                relax_maker_dp=self.relax_maker_dp,
...
class IOMaker(Maker):
    ...
    #: ``pairs_summary``). **Default ``each_match_lowest``:** after MLIP, run VASP for the **lowest-energy
    #: termination per lattice match** (one pair per ``match_id``). ``each_plane_lowest`` keeps one winner per
    #: film/substrate Miller bin (aligned with stereographic / ``area_strain`` binning, often fewer interface relax
    #: jobs than ``each_match_lowest``). ``all_optimized`` runs VASP for **every** row in ``global_optimized_data``.
    #: Aliases: ``each_match`` / ``per_match_lowest`` → ``each_match_lowest``; ``each_plane`` / ``per_plane`` → ``each_plane_lowest``.
    ...
    def _resolved_pair_selection_kwargs(self) -> Dict[str, Any]:
        ...
        if sel in ("all_optimized", "all", "all_pairs"):
        ...
        elif sel in ("each_match_lowest", "per_match_lowest", "each_match"):
            base = {
                "all_optimized_pairs": False,
                "only_lowest_energy": False,
                "only_lowest_energy_each_plane": False,
            }
        elif sel in ("each_plane_lowest", "per_plane_lowest", "each_plane", "per_plane"):
            base = {
                "all_optimized_pairs": False,
                "only_lowest_energy": False,
            ...
            raise ValueError(
                ...
                "use 'each_match_lowest' (default), 'each_plane_lowest', or 'all_optimized'. "
                "Aliases: each_match, per_match_lowest; each_plane, per_plane (legacy)."
            )
        return dict(base)
    ...
    def IO_HT_job(self, film_conv, substrate_conv):
        ...
        # Stereographic plots / ``area_strain`` need one representative energy per Miller bin
        # (``only_lowest_energy_each_plane`` in :meth:`~InterfaceWorker.visualize_minimization_results`).
        # That is **independent** of which interfaces VASP relaxes; see ``pair_kw`` below.
        ...
        for (i, j) in pairs:
            ...
            try:
                ...
                film_hkl, sub_hkl = _miller_hkl_lists_for_report(idx_data)
            except Exception:
                film_hkl, sub_hkl = None, None
