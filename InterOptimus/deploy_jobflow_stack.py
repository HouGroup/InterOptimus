...
可选 --with-interoptimus：在当前 conda 环境 pip install -e 安装 InterOptimus，并创建 conda 环境 orb / dpa / matris / sevenn，
...
用法示例（与本机 Mongo 库 htp_test / 用户 htp_test 一致；默认 --mongo-host auto）:
  python deploy_jobflow_stack.py \\
    --mongo-db htp_test --mongo-user htp_test --mongo-password htp_test
...
仅检测（不写入）:
  python deploy_jobflow_stack.py --verify-only --mongo-db htp_test \\
    --mongo-user htp_test --mongo-password htp_test
"""
...
def main() -> None:
    ...
    parser.add_argument("--mongo-password", default="")
    parser.add_argument(
        "--mongo-auth-source",
        default=None,
    ...
    parser.add_argument(
        ...
        help="主 local worker 名称（atomate2/通用任务）；与 --with-interoptimus 并存时为默认 worker 之一",
    )
    parser.add_argument(
        "--vasp-cmd",
        default="vasp_std",
    ...
    parser.add_argument(
        "--skip-mlip-conda",
        action="store_true",
        help="与 --with-interoptimus 联用：跳过本机 pip install -e 与四个 MLIP conda，仅把 worker 写入项目 YAML",
    )
    parser.add_argument(
        "--matris-local",
        type=Path,
    ...
    if args.skip_mlip_conda and not args.with_interoptimus:
        parser.error("--skip-mlip-conda 需与 --with-interoptimus 同时使用")
    ...
    with_io = args.with_interoptimus
    if with_io:
        if not _conda_exe_or_none():
            _die("--with-interoptimus 需要 conda 在 PATH 中（登录节点先 conda activate）")
    ...
    workers = _build_jfremote_workers(
        ...
        with_interoptimus_mlips=with_io,
    )
    ...
    if with_io:
        print("  InterOptimus: 已在 jfremote 中配置 MLIP workers：orb, dpa, matris, sevenn（conda 环境同名）")
