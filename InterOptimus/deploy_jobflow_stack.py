...
不走 ``download.pytorch.org``，方便内网 / 离线集群通过 PyPI 镜像源安装。需要其它精确版本可用 --torch-spec 'torch==2.10.0' 等覆盖。
...
# PyPI 默认 torch wheel 在 2.11 起切换为 CUDA 13；2.7..2.10.x 仍是 CUDA 12 系列（含 12.4/12.6/12.8）。
# 钉死在 2.10.x 区间，避免离线/内网镜像被拉到 CUDA 13 wheel；同时这些版本仍保留 Volta/V100 支持。
TORCH_PIN_DEFAULT = "torch>=2.7,<2.11"
...
def main() -> None:
    ...
    parser.add_argument(
        "--torch-spec",
        default=TORCH_PIN_DEFAULT,
        help=(
            "传给 ``pip install`` 的 torch 版本约束字符串（不走 download.pytorch.org，便于内网/离线）。"
            f"默认 {TORCH_PIN_DEFAULT!r} —— 该范围内 PyPI 默认 wheel 为 CUDA 12 系列；"
            "如要更精确可写 'torch==2.10.0' 等。"
        ),
    )
