...
MatRIS 优先顺序：--matris-local → 环境变量 INTEROPTIMUS_MATRIS_LOCAL / MATRIS_LOCAL_PACKAGE → ~/software 中含 matris 的 .zip（自动）→ InterOptimus 同级/子目录 MatRIS；不会从 git 自动安装。
...
MLIP_CONDA_SPECS: list[dict[str, Any]] = [
    ...
    {"worker": "matris", "env": "matris", "pip": []},  # MatRIS 由 resolve_matris_install_source 决定本地来源
    {"worker": "sevenn", "env": "sevenn", "pip": ["sevenn"]},
]
MLIP_WORKER_KEYS = tuple(spec["worker"] for spec in MLIP_CONDA_SPECS)


def parse_mlip_worker_selection(selection: str) -> list[dict[str, Any]]:
    """Return selected MLIP conda/worker specs from ``all`` or comma-separated worker names."""
    raw = (selection or "all").strip().lower()
    if raw in {"all", "*"}:
        keys = set(MLIP_WORKER_KEYS)
    else:
        keys = {part.strip().lower() for part in raw.split(",") if part.strip()}
    bad = sorted(keys - set(MLIP_WORKER_KEYS))
    if bad:
        raise ValueError(
            "未知 MLIP worker: "
            + ", ".join(bad)
            + f"；可选: all 或 {','.join(MLIP_WORKER_KEYS)}"
        )
    return [spec for spec in MLIP_CONDA_SPECS if spec["worker"] in keys]
...
def resolve_matris_install_source(
    ...
    → ~/software 中含 matris 的 .zip（自动）→ 常见本地目录。不会从 git 自动安装。
    """
    ...
    _die(
        "选择了 MatRIS worker，但未找到可用的本地 MatRIS 包/源码。\n"
        "请使用 --matris-local 指定 MatRIS wheel/tar.gz/zip/源码目录，或设置 "
        "INTEROPTIMUS_MATRIS_LOCAL / MATRIS_LOCAL_PACKAGE，或把文件名含 matris 的 zip 放到 ~/software。\n"
        "如果当前服务器不能访问 git，建议先只配置其它 MLIP，例如 --mlip-workers orb。"
    )
...
def _setup_mlip_conda_envs(
    ...
    *,
    ...
    """为选定 MLIP 创建 conda 环境并安装对应 MLIP 与 InterOptimus（无其它 MLIP 依赖）。"""
    _conda_exe()
    matris_arg = ""
    if any(spec["worker"] == "matris" for spec in selected_specs):
...
def _build_jfremote_workers(
    *,
    ...
    selected_mlip_specs: list[dict[str, Any]] | None = None,
) -> dict[str, Any]:
    ...
    if with_mlip_workers:
        for spec in selected_mlip_specs or MLIP_CONDA_SPECS:
            w = spec["worker"]
            env = spec["env"]
...
def _apply_interactive_config(args: argparse.Namespace) -> None:
    ...
    args.with_mlip_workers = _prompt_bool("是否配置 MLIP workers", default=args.with_mlip_workers)
    if args.with_mlip_workers:
        args.mlip_workers = _prompt_text(
            "需要配置的 MLIP workers（all 或 orb,sevenn,dpa,matris）",
            default=args.mlip_workers,
            required=True,
        )
        if not args.skip_mlip_conda:
            if "matris" in {part.strip().lower() for part in str(args.mlip_workers).split(",")} or str(args.mlip_workers).strip().lower() in {"all", "*"}:
                matris = _prompt_text("MatRIS 本地包/源码路径（可留空，自动查 ~/software；找不到则不安装 MatRIS）", default=str(args.matris_local or ""))
        ...
        args.checkpoint_models = _prompt_text(
            ...
            default=args.checkpoint_models if args.checkpoint_models != "all" else args.mlip_workers,
            required=True,
        )
    ...
    if args.with_mlip_workers:
        ...
        print(f"  MLIP selected workers: {args.mlip_workers}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--with-mlip-workers",
        action="store_true",
        help="安装 InterOptimus 并配置选定 MLIP conda 环境、checkpoint 与 jfremote worker",
    )
    parser.add_argument(
        "--mlip-workers",
        default="all",
        help="与 --with-mlip-workers 联用：all 或逗号分隔 orb,sevenn,dpa,matris；例如只装 ORB: --mlip-workers orb",
    )
    parser.add_argument(
        "--checkpoint-models",
        default="all",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="MatRIS 本地路径：wheel/tar.gz/zip 或源码目录；未设时查环境变量、~/software 中含 matris 的 .zip（自动）、InterOptimus 旁 MatRIS；不会从 git 安装",
    )
    ...
    if str(args.mlip_workers).strip().lower() not in {"all", "*"} and not args.with_mlip_workers:
        parser.error("--mlip-workers 需与 --with-mlip-workers 同时使用")
    ...
    checkpoint_specs = []
    selected_mlip_specs: list[dict[str, Any]] = []
    if with_mlip_workers:
        from InterOptimus.checkpoints import (
        ...
        try:
            selected_mlip_specs = parse_mlip_worker_selection(args.mlip_workers)
        except ValueError as exc:
            parser.error(str(exc))
        selected_mlip_workers = [spec["worker"] for spec in selected_mlip_specs]
        if str(args.checkpoint_models).strip().lower() in {"all", "*"} and str(args.mlip_workers).strip().lower() not in {"all", "*"}:
            args.checkpoint_models = ",".join(selected_mlip_workers)
        try:
            checkpoint_specs = parse_checkpoint_selection(args.checkpoint_models)
        except ValueError as exc:
            parser.error(str(exc))
        ...
        if not args.skip_mlip_conda:
            ...
            _setup_mlip_conda_envs(
                ...
                selected_specs=selected_mlip_specs,
            )
        else:
            print(
    ...
    workers = _build_jfremote_workers(
        ...
        selected_mlip_specs=selected_mlip_specs,
    )
    ...
    if with_mlip_workers:
        configured = ", ".join(spec["worker"] for spec in selected_mlip_specs)
        print(f"  MLIP workers: 已在 jfremote 中配置 {configured}（conda 环境同名）")
        if checkpoint_specs:
