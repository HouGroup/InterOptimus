...
def _iomaker_interface_static_tag_required(
    mi: int,
    ti: int,
    job_by_tag: Optional[Dict[str, Any]] = None,
    *,
    extra_tag_keys: Optional[Any] = None,
) -> str:
    """VASP job tag for the post-relax interface ``MPStaticSet`` (``{m}_{t}_it``)."""
    return f"{mi}_{ti}_it"


def _iomaker_cohesive_slab_static_tag_required(
    mi: int,
    ti: int,
    slab: str,
    job_by_tag: Optional[Dict[str, Any]] = None,
    *,
    ...
    """Slab reference tag for cohesive γ (post-relax ``MPStaticSet``)."""
    return f"{mi}_{ti}_{slab}_slab"


def _iomaker_interface_energy_eV(
    energies_by_tag: Dict[str, float], mi: int, ti: int
) -> Optional[float]:
    """Interface total energy: canonical static ``{m}_{t}_it``, then legacy ``{m}_{t}_it_ismear2``."""
    base = f"{mi}_{ti}_it"
    e = _lookup_tag_energy(energies_by_tag, base)
    if e is not None:
        return e
    return _lookup_tag_energy(energies_by_tag, f"{base}_ismear2")


def _iomaker_cohesive_slab_energy_eV(
    energies_by_tag: Dict[str, float], mi: int, ti: int, slab: str
) -> Optional[float]:
    base = f"{mi}_{ti}_{slab}_slab"
    e = _lookup_tag_energy(energies_by_tag, base)
    if e is not None:
        return e
    return _lookup_tag_energy(energies_by_tag, f"{base}_ismear2")
...
def _iomaker_double_interface_bulk_ref_tags(mapping: Optional[Dict[str, Any]]) -> Tuple[str, str]:
    """Primitive bulk reference tags for double-interface γ."""
    return "film", "substrate"


def _iomaker_required_vasp_tags(
    mi: int, ti: int, *, double_it: bool, job_by_tag: Optional[Dict[str, Any]] = None
) -> List[str]:
    """VASP job name tags (``metadata['job']``) required to assemble one pair's interface energy."""
    if double_it:
        ft, st = _iomaker_double_interface_bulk_ref_tags(job_by_tag)
        return [ft, st, _iomaker_interface_static_tag_required(mi, ti, job_by_tag)]
    return [
        _iomaker_interface_static_tag_required(mi, ti, job_by_tag),
        _iomaker_cohesive_slab_static_tag_required(mi, ti, "film", job_by_tag),
        _iomaker_cohesive_slab_static_tag_required(mi, ti, "substrate", job_by_tag),
    ]


# Any VASP job whose ``metadata.job`` starts with ``{m}_{t}_it_`` or equals ``{m}_{t}_it`` schedules that pair.
_VASP_INTERFACE_IT_TAG_RE = re.compile(r"^(\d+)_(\d+)_it(?:_|$)")
...
def _scheduled_interface_pairs_from_vasp_tags(
    ...
    **Stereographic ``area_strain``** (see ``InterfaceWorker.visualize_minimization_results``) still uses
    ...
    need not line up 1:1 with submitted ``*_it`` tags unless you set ``vasp_pair_selection='each_plane_lowest'``.
...
def _iomaker_pair_vasp_job_rollup(
    ...
    *,
    ...
    """
    for tg in _iomaker_required_vasp_tags(mi, ti, double_it=double_it, job_by_tag=job_by_tag):
...
def _iomaker_collect_vasp_energy_rows(
    ...
    *,
    ...
    _bulk_f, _bulk_s = _iomaker_double_interface_bulk_ref_tags(job_by_tag)
    e_f0 = _lookup_tag_energy(energies_by_tag, _bulk_f)
    e_s0 = _lookup_tag_energy(energies_by_tag, _bulk_s)
    ...
    for (mi, ti), od in sorted(opt_results.items(), key=lambda x: (x[0][0], x[0][1])):
        ...
        e_it = _iomaker_interface_energy_eV(energies_by_tag, mi, ti)
        vasp_energy = None
        ...
        if double_it:
            ...
            e_film = _iomaker_cohesive_slab_energy_eV(energies_by_tag, mi, ti, "film")
            if e_film is None:
                e_film = energies_by_tag.get(f"{mi}_{ti}_film")
            e_sub = _iomaker_cohesive_slab_energy_eV(energies_by_tag, mi, ti, "substrate")
            if e_sub is None:
...
def _global_rows_by_pair_for_csv(payload: Dict[str, Any]) -> Dict[Tuple[int, int], Dict[str, Any]]:
    rows = payload.get("global_optimized_data") if isinstance(payload, dict) else None
...
def _global_rows_by_pair_from_all_data_csv(dest_root: str) -> Dict[Tuple[int, int], Dict[str, Any]]:
    out: Dict[Tuple[int, int], Dict[str, Any]] = {}
    for path in sorted(Path(dest_root).glob("all_data*.csv")):
        if not path.is_file():
            continue
        try:
...
def _global_energy_column_for_csv(row: Dict[str, Any]) -> str:
    for key in row:
...
def _area_strain_planes_nonempty(rec: Dict[str, Any]) -> bool:
    """True when both Miller triplets look usable (from :func:`parse_area_strain_records`)."""
    p1 = rec.get("material1_plane") or []
    p2 = rec.get("material2_plane") or []
    try:
        return len(p1) >= 3 and len(p2) >= 3
    except Exception:
        return False


def _best_area_strain_record_for_pair(
    records: List[Dict[str, Any]], pair: Tuple[int, int]
) -> Dict[str, Any]:
    """
    Pick one ``area_strain`` row for ``(match_id, term_id)`` with the best Miller labels.

    Stereographic output often repeats the same pair with ``stereo_winner`` 0/1 and equivalent
    Miller labelings; the old logic only scanned ``stereo_winner != 0`` rows, so many DFT pairs
    fell through to ``{}`` and showed ``[]`` in ``pairs_summary`` / ``io_report``.
    """
    best: Optional[Dict[str, Any]] = None
    for record in records:
        if record.get("term_id") is None:
            continue
        if (int(record["match_id"]), int(record["term_id"])) != pair:
            continue
        if best is None:
            best = record
        elif not _area_strain_planes_nonempty(best) and _area_strain_planes_nonempty(record):
            best = record
    return best or {}


def _best_area_strain_record_for_match(
    records: List[Dict[str, Any]], match_id: int
) -> Dict[str, Any]:
    """
    Any ``area_strain`` row for this ``match_id`` with usable Miller triplets.

    ``visualize_minimization_results`` often tags many lines with the **same** ``(match_id, term_id)``
    (the bin winner only); other terminations for that match may have **no** line with their exact
    ``(match_id, term_id)``, but film/substrate Miller indices are shared across terminations.

    Legacy rows may omit ``term_id``; they are still used here so match-level Millers can be filled.
    """
    best: Optional[Dict[str, Any]] = None
    for record in records:
...
def _planes_from_area_strain_record(rec: Dict[str, Any]) -> Tuple[List[int], List[int]]:
    m1 = rec.get("material1_plane") or []
    m2 = rec.get("material2_plane") or []
    return (
        [int(round(float(x))) for x in m1],
        [int(round(float(x))) for x in m2],
    )


def _miller_triplets_from_unique_matches_table(
    um: Optional[List[Dict[str, Any]]], match_id: int
) -> Tuple[List[int], List[int]]:
    """Film/substrate Miller lists from ``opt_results.pkl`` or ``unique_matches_millers.json`` sidecar."""
    if not isinstance(um, list) or not (0 <= match_id < len(um)):
        return [], []
    block = um[match_id]
    if not isinstance(block, dict):
        return [], []
    fc = block.get("film_conventional_miller") or []
    sc = block.get("substrate_conventional_miller") or []
    if not fc or not sc:
        return [], []
    try:
        return [int(round(float(x))) for x in fc], [int(round(float(x))) for x in sc]
    except (TypeError, ValueError):
        return [], []
...
def _decode_monty_numpy_json(obj: Any) -> Any:
    """Turn Monty JSON dicts (e.g. numpy arrays in ``io_flow.json``) into Python / numpy objects."""
    if isinstance(obj, dict):
        if obj.get("@module") == "numpy" and obj.get("@class") == "array":
            import numpy as np

            return np.asarray(obj.get("data"), dtype=float)
        return {k: _decode_monty_numpy_json(v) for k, v in obj.items()}
    if isinstance(obj, list):
        return [_decode_monty_numpy_json(x) for x in obj]
    return obj


def _sanitize_lattice_matching_miller_arrays(lm: Dict[str, Any]) -> Dict[str, Any]:
    """Ensure ``film_millers`` / ``substrate_millers`` are integer arrays (pymatgen SlabGenerator)."""
    import numpy as np

    out = dict(lm)
    for key in ("film_millers", "substrate_millers"):
        if key not in out or out[key] is None:
            continue
        a = np.asarray(out[key], dtype=float)
        if a.size:
            out[key] = np.round(a).astype(int)
    return out


def _lattice_matching_settings_from_io_flow_json(path: str) -> Dict[str, Any]:
    """Read ``lattice_matching_settings`` from a serialized jobflow ``io_flow.json`` (raw JSON, no full Monty IOMaker)."""
    raw = json.loads(Path(path).read_text(encoding="utf-8"))
    bound = raw["jobs"][0]["function"]["@bound"]
    lm = bound.get("lattice_matching_settings")
    if not isinstance(lm, dict):
        return {}
    return _sanitize_lattice_matching_miller_arrays(_decode_monty_numpy_json(lm))


def _find_io_flow_json_near_dest_root(dest_root: str) -> Optional[str]:
    """
    Locate ``io_flow.json`` next to exported results (e.g. ``FeS2/00dd7cef_results`` → ``FeS2/FeS2_Caoyun``).
    Used to regenerate ``unique_matches_millers.json`` when VASP merge would otherwise show ``[]`` Millers.
    """
    root = os.path.abspath(dest_root)
    for rel in ("io_flow.json",):
    ...
    try:
        for name in sorted(os.listdir(base)):
            ...
            if not os.path.isdir(sub):
                continue
            if name.endswith("_Caoyun") or "Caoyun" in name:
                preferred.append(sub)
            else:
                rest.append(sub)
    except OSError:
        return None
    for sub in preferred + rest:
        p = os.path.join(sub, "io_flow.json")
        if os.path.isfile(p):
            return p
    return None


def _ensure_umtbl_for_vasp_miller_columns(
    dest_root: str,
    pair_row: Dict[Tuple[int, int], Dict[str, Any]],
    records: List[Dict[str, Any]],
    um_tbl: Optional[List[Dict[str, Any]]],
) -> Optional[List[Dict[str, Any]]]:
    """
    ``area_strain`` often lists only stereographic-bin winners, so many ``(match_id, term_id)`` DFT rows
    have no line. Fallback is ``unique_matches_millers`` from pickle or ``unique_matches_millers.json``.
    If that table is missing or has empty Miller entries, regenerate it via lattice matching + ``io_flow.json``.
    """
    for pair, hit in pair_row.items():
        if hit.get("vasp_pair_status") == "mlip_only":
            continue
        fm, sm = _film_sub_miller_for_vasp_pair_row(pair, records, um_tbl)
        if not fm or not sm:
            break
    else:
        return um_tbl
    io_flow = _find_io_flow_json_near_dest_root(dest_root)
    if not io_flow:
        return um_tbl
    reg = iomaker_write_unique_matches_millers_sidecar(dest_root, io_flow_json=io_flow)
    if not reg.get("success"):
        return um_tbl
    return _load_unique_matches_millers_sidecar(dest_root) or um_tbl
...
def _read_cifs_from_export_io_reports(dest_root: str) -> Tuple[str, str]:
    ...
    for rel in ("io_report.txt", "vasp_results/io_report.txt"):
        ...
        try:
            film_cif = ""
            substrate_cif = ""
            for raw_line in Path(p).read_text(encoding="utf-8", errors="replace").splitlines():
                line = raw_line.strip()
                if line.startswith("Film CIF:"):
                    film_cif = line.split(":", 1)[1].strip()
                elif line.startswith("Substrate CIF:"):
                    substrate_cif = line.split(":", 1)[1].strip()
            film_path = _iomaker_existing_path(film_cif, base_dir=dest_root)
            substrate_path = _iomaker_existing_path(substrate_cif, base_dir=dest_root)
            if film_path and substrate_path:
                return film_path, substrate_path
        except Exception:
            continue
...
def iomaker_write_unique_matches_millers_sidecar(
    dest_dir: str,
    *,
    film_cif: Optional[str] = None,
    substrate_cif: Optional[str] = None,
    lattice_matching_settings: Optional[Dict[str, Any]] = None,
    io_flow_json: Optional[str] = None,
) -> Dict[str, Any]:
    """
    For **completed** runs whose ``opt_results.pkl`` has no ``unique_matches_millers`` field:
    write ``unique_matches_millers.json`` next to ``opt_results.pkl`` by running **lattice matching only**
    (no MLIP/VASP). Then call :func:`iomaker_fetch_results` again (or ``iomaker_build_vasp_mlip_style_results``)
    to fill Miller columns in ``pairs_summary`` / ``io_report``.

    Provide ``io_flow_json`` (same ``io_flow.json`` as submission) to recover ``lattice_matching_settings``.
    If ``film_cif`` / ``substrate_cif`` are omitted, they are read from ``io_report.txt`` under *dest_dir*.
    """
    from InterOptimus.jobflow import _serialize_unique_matches_millers_for_payload
    from InterOptimus.itworker import InterfaceWorker
    from pymatgen.core.structure import Structure

    dest_root = os.path.abspath(dest_dir)
    if not os.path.isdir(dest_root):
        return {"success": False, "error": "missing_dest_dir", "dest_dir": dest_root}

    if io_flow_json and lattice_matching_settings is None:
        try:
            lattice_matching_settings = _lattice_matching_settings_from_io_flow_json(io_flow_json)
        except Exception as e:
            return {
                "success": False,
                "error": "io_flow_parse_failed",
                "hint": str(e),
                "dest_dir": dest_root,
            }

    if not lattice_matching_settings:
        return {
            "success": False,
            "error": "missing_lattice_matching_settings",
            "hint": "Pass io_flow_json=... or lattice_matching_settings={...}.",
            "dest_dir": dest_root,
        }
    lattice_matching_settings = _sanitize_lattice_matching_miller_arrays(lattice_matching_settings)

    if not film_cif or not substrate_cif:
        fc, sc = _read_cifs_from_export_io_reports(dest_root)
        film_cif = film_cif or fc
        substrate_cif = substrate_cif or sc
    ...
    film_conv = Structure.from_file(film_path)
    substrate_conv = Structure.from_file(sub_path)
...
def _film_sub_miller_for_vasp_pair_row(
    pair: Tuple[int, int],
    records: List[Dict[str, Any]],
    unique_matches_millers: Optional[List[Dict[str, Any]]],
) -> Tuple[List[int], List[int]]:
    """Resolve Miller columns for ``pairs_summary`` / ``io_report`` (exact pair → match → table)."""
    sample = _best_area_strain_record_for_pair(records, pair)
    if _area_strain_planes_nonempty(sample):
        return _planes_from_area_strain_record(sample)
    sample_m = _best_area_strain_record_for_match(records, pair[0])
    if _area_strain_planes_nonempty(sample_m):
        return _planes_from_area_strain_record(sample_m)
    return _miller_triplets_from_unique_matches_table(unique_matches_millers, pair[0])
...
def iomaker_build_vasp_mlip_style_results(
    ...
    *,
    ...
    for record in vasp_area_records:
        ...
        sw = int(record.get("stereo_winner", 1))
        line = (
            f"({p1[0]} {p1[1]} {p1[2]}) "
            f"({p2[0]} {p2[1]} {p2[2]}) "
            f"{float(record['area']):.4f} {float(record['strain']):.4f} "
            f"{float(record['binding_energy']):.4f} {int(record['match_id'])} {int(record['term_id'])} {sw}"
        )
    ...
    for pair in sorted(pair_row.keys()):
        ...
        if hit.get("vasp_pair_status") == "mlip_only":
            continue
        film_m, sub_m = _film_sub_miller_for_vasp_pair_row(pair, records, um_tbl)
        ...
        pair_summary_rows.append(
            {
                "match_id": pair[0],
                "term_id": pair[1],
                "film_conventional_miller": film_m,
                "substrate_conventional_miller": sub_m,
    ...
    def _vasp_csv_sort_key(row: Dict[str, Any]) -> Tuple[float, int, int]:
        try:
            energy = float(row.get("energy_value"))
        except (TypeError, ValueError):
            energy = float("inf")
        return (energy, int(row["match_id"]), int(row["term_id"]))

    pair_summary_rows.sort(key=_vasp_csv_sort_key)
    ...
    vasp_csv_path = os.path.join(vasp_root, "selected_interfaces.csv")
    global_rows_for_csv = _global_rows_by_pair_for_csv(pkl_payload if isinstance(pkl_payload, dict) else {})
    if not global_rows_for_csv:
        global_rows_for_csv = _global_rows_by_pair_from_all_data_csv(dest_root)
    vasp_csv_rows: List[Dict[str, Any]] = []
    for row in pair_summary_rows:
        mi = int(row["match_id"])
        ti = int(row["term_id"])
        out_row = dict(global_rows_for_csv.get((mi, ti), {"$i_m$": mi, "$i_t$": ti}))
        energy_col = _global_energy_column_for_csv(out_row)
        if energy_col:
            out_row[energy_col] = row["energy_value"]
        ...
        out_row["cif"] = best_it.to(fmt="cif") if best_it is not None else ""
        vasp_csv_rows.append(out_row)

    csv_fields: List[str] = []
    seen_csv_fields: set[str] = set()
    for row in vasp_csv_rows:
        for key in row:
            if key not in seen_csv_fields:
                seen_csv_fields.add(key)
                csv_fields.append(key)
    if "cif" in csv_fields:
        csv_fields = [c for c in csv_fields if c != "cif"] + ["cif"]
    elif not csv_fields:
        csv_fields = ["cif"]
    with open(vasp_csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=csv_fields, extrasaction="ignore")
        writer.writeheader()
        for out_row in vasp_csv_rows:
    ...
    report_lines.extend(
        [
            "Pairs (DFT-scheduled only; MLIP-only terminations are in opt_results.pkl)",
    ...
    for obsolete in (io_report_path, pairs_summary_path, vasp_summary_path):
        if obsolete and os.path.isfile(obsolete):
            try:
                os.remove(obsolete)
            except OSError:
                pass
    if os.path.isdir(vasp_pairs_best_it_dir):
        shutil.rmtree(vasp_pairs_best_it_dir, ignore_errors=True)
...
def iomaker_build_vasp_interface_report(
    ...
    *,
    ...
    j_per_m2 = 16.02176634
    _bulk_f, _bulk_s = _iomaker_double_interface_bulk_ref_tags(energies_by_tag)
    e_f0 = _lookup_tag_energy(energies_by_tag, _bulk_f)
    e_s0 = _lookup_tag_energy(energies_by_tag, _bulk_s)
    rows_out: List[Dict[str, Any]] = []
    for (mi, ti), od in sorted(opt_results.items(), key=lambda x: (x[0][0], x[0][1])):
...
def iomaker_fetch_results(
    ...
    *,
    ...
    if verbose:
        ...
        if fr.get("mlip_results_dir"):
            print(" mlip_results:", fr.get("mlip_results_dir"))
...
def fetch_interoptimus_task_results(
    ...
    *copy_images_to*, if set, copies into that directory (created if missing):
    ...
    if copy_images_to and collected.get("success") and collected.get("run_dir"):
        ...
        for src_path in sorted(Path(rd).glob("all_data*.csv")):
            if src_path.is_file():
                dst = os.path.join(dest_root, src_path.name)
                shutil.copy2(str(src_path), dst)
                copied_art.append({"from": str(src_path), "to": dst, "kind": "file"})
        if copied_art:
