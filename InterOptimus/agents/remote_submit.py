...
def submit_io_flow_locally(
    ...
    *remote_python* defaults to :data:`sys.executable` (the interpreter running this code), or
    ``INTEROPTIMUS_SERVER_PYTHON`` when that env var is set. Pass an explicit path (e.g. ``\"python\"``)
    only if you need a different interpreter on the login node.
    """
    ...
    py_exe = (remote_python or "").strip()
    if not py_exe:
        py_exe = (os.getenv("INTEROPTIMUS_SERVER_PYTHON") or "").strip() or sys.executable
    inner = f"""set -e
cd {shlex.quote(str(wd))}
{pre_cmd}
export PYTHONPATH={shlex.quote(str(source_root))}:${{PYTHONPATH:-}}
{shlex.quote(py_exe)} - <<'PY'
{script_body}
PY
"""
    try:
...
if __name__ == "__main__":
    ...
    print(
        ...
        "or the LLM entry point InterOptimus.agents.llm_iomaker_job.build_iomaker_flow_from_prompt.",
        file=sys.stderr,
    )
